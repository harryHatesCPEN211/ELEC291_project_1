...
FSM_State3_Check:
    ...
    jc    NotGreater          ; if carry => Oven_Reading.high < reflow_temp.high => ret
    jnz   IsGreater           ; if A != 0 => Oven_Reading.high > reflow_temp.high => jump
    
    ; 2) High bytes are equal; compare the low bytes
    mov   A, Oven_Display+1   ; A = Oven_Reading.low
    clr   C
    subb  A, reflow_temp+0    ; A = (Oven_Reading.low) - (reflow_temp.low)
    jc    NotGreater          ; if carry => Oven_Reading.low < reflow_temp.low => ret
    jz    NotGreater          ; if zero => values are equal => not greater => ret
